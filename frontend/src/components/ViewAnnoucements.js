...
const ViewAnnoucements = ({ club, showChat, setShowChat }) => {
  ...
  return (
    <div className=" text-black w-[60vw] ml-40 m-8 font-semibold border-2 rounded-md">
      <ToastContainer
        position="top-right"
        autoClose={2000}
      ...
      {!showChat &&
        annouce.map((ele, i) => {
          if (ele?.isPrivate === false) {
              ...
              >
                ...
                <div className="w-full" style={{ display: "flex", alignItems: "center"  }}>
                  <IconButton onClick={() => handleLike(ele._id)}>
                    <h1 className="text-md mr-4">Are you interested</h1>
            ...
            ele?.isPrivate === true &&
            user.user?.club?.includes(club) === true
          ) {
            // console.log(ele)
            return (
              <div key={i} className="">
      ...
      {!showChat && (
        <button
          className="absolute top-20 p-4 text-white w-40 rounded-full shadow-lg text-xl bg-sky-500 right-10"
